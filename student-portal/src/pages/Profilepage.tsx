            ...
            {isGuardian ? (
              <div className="relative flex flex-col items-center">
                <div className="relative w-20 h-20 rounded-2xl border-4 border-white shadow-xl bg-green-50
                                flex items-center justify-center shrink-0 overflow-hidden -mt-10 z-10">
                  {displayProfile?.image ? (
                    <img src={displayProfile.image} alt={displayProfile.student_name} className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-2xl font-extrabold text-primary-green">
                      {getInitials(displayProfile?.student_name || 'S')}
                    </span>
                  )}
                </div>
              </div>
