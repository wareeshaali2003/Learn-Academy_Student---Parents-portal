...
export const Login: React.FC = () => {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      let msg = error.response?.data?.message || error.message || "Login failed. Please check your credentials.";
      if (error.message === 'Network Error') {
        msg = 'Network Error: This is likely a CORS issue. Please ensure your ERPNext server allows requests from this domain.';
      }
      setErr(msg);
    } finally {
      setBusy(false);
  ...
  const onForgotSubmit = async () => {
    ...
    setForgotBusy(true);
    try {
      const res = await erpService.resetPassword(forgotEmail.trim());
      if (res.ok) {
        setForgotSuccess(true);
      } else {
        setForgotErr(res.error || "Failed to send reset link.");
      }
    } catch {
  ...
  return (
    <div style={{
      ...
      {/* ── Left branding panel ── */}
      <div className="hidden md:flex flex-1 flex-col justify-center p-16 relative z-10" style={{
        opacity: mounted ? 1 : 0,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {[
            { icon: "◈", text: "Attendance & Results" },
            { icon: "◉", text: "Assignments & Quizzes" },
      ...
      {/* ── Right login panel ── */}
      <div className="w-full md:w-[42%] min-w-[320px] flex items-center justify-center p-6 relative z-10">
        <div style={{
